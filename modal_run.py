...
APP_NAME = "mal-cifar10-wandb-sweep"
SWEEP_ID = "osuwaidi-khalifa-university/FINAL_MAL_CIFAR10/udwhpe1h"
...
def _terminate_agent(agent: subprocess.Popen[bytes]) -> None:
    """Stop W&B cleanly so its child training run is marked preempted."""
    if agent.poll() is not None:
        return

    print("Credit-aware runtime reached; stopping the W&B agent cleanly.", flush=True)
...
@app.function(
    ...
    cpu=CPU_CORES,
    memory=MEMORY_MIB,
    timeout=DEFAULT_AGENT_SECONDS + SHUTDOWN_GRACE_SECONDS + 60,
    secrets=[wandb_secret],
)
def run_sweep_agent(count: int = 0, max_runtime_seconds: int = DEFAULT_AGENT_SECONDS) -> int:
    """Run one W&B agent; ``count=0`` means until sweep completion/budget expiry."""
    if count < 0:
        raise ValueError("count must be zero (unlimited) or a positive integer")
    if not 1 <= max_runtime_seconds <= DEFAULT_AGENT_SECONDS:
        raise ValueError(f"max_runtime_seconds must be between 1 and {DEFAULT_AGENT_SECONDS}")
    ...
    budget_hours = max_runtime_seconds / 3600
    estimated_cost = max_runtime_seconds * TOTAL_RATE
    print(
        f"Starting W&B sweep agent on {gpu_name}. "
        f"Runtime guard: {budget_hours:.2f} h (~${estimated_cost:.2f}).",
        flush=True,
    )
...
def main(count: int = 0, max_hours: float = DEFAULT_AGENT_SECONDS / 3600) -> None:
    ...
    return_code = run_sweep_agent.remote(
        count=count,
